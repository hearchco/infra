...
# Afterwards use Find and Replace to replace all occurrences "eu-west-2" with the new region name.
# Lastly, use Find and Replace to replace all occurrences "eu_west_2" with the new region name.
...
module "hearchco_lambda_eu_west_2" {
  source           = "../../modules/backend/lambda"
  role             = module.lambda_iam.role_arn
  s3_bucket        = module.hearchco_s3_eu_west_2.bucket_id
  s3_key           = module.hearchco_s3_eu_west_2.s3_key
  source_code_hash = module.hearchco_s3_eu_west_2.source_code_hash
...
module "hearchco_certificate_eu_west_2" {
...
module "hearchco_apigateway_eu_west_2" {
  source              = "../../modules/backend/apigateway"
  domain_name         = local.api_gateway_domain_name
  hosted_zone_id      = data.aws_route53_zone.hearchco_route53.zone_id
  acm_certificate_arn = module.hearchco_certificate_eu_west_2.cert_arn
  function_name       = module.hearchco_lambda_eu_west_2.function_name
  invoke_arn          = module.hearchco_lambda_eu_west_2.invoke_arn
  routes              = local.routes
